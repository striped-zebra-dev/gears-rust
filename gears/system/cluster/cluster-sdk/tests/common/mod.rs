...
use cluster_sdk::{
    CacheDescriptor, ClusterClient, LeaderElectionDescriptor, LeaderElectionFeaturesDto,
    LockDescriptor, LockFeaturesDto, ProfileDescriptor, ProfileHealth,
};
...
/// (DESIGN-DEPLOYABLE-GEAR §4.9.3), so a smoke test that wants a resolvable
